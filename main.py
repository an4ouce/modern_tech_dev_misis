...
def check_pwd():
    ...
    while True:
        ...
        if len(pwd) < 8:
            ...
            print("-----------------------------------------------")
        elif (all(char in special_chars for char in pwd)):
            print("Пароль не может состоять только из спецсимволов!")
            print("-----------------------------------------------")
        elif not (any(char in digits for char in pwd)):
            print("Пароль не может состоять только из букв!")
            print("-----------------------------------------------")
        elif not (any(char in letters for char in pwd)):
            print("Пароль не может состоять только из цифр!")
            print("-----------------------------------------------")
        elif not (any(char in letters[26:] for char in pwd)):
            print("В пароле хотя бы одна буква должна быть в верхнем регистре!")  
            print("-----------------------------------------------")
        elif not (any(char in special_chars for char in pwd)):
